...
fun Modifier.pdfPageGestureModifier(
    ...
    fun resetGestureState(forceEndTransform: Boolean = false) {
        if ((isTransforming || forceEndTransform)) {
            ...
            object : GestureDetector.SimpleOnGestureListener() {
                ...
                override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
                    if (!isTransforming && !suppressNextTap) {
                        ...
                        // pinch 종료 직후 첫 탭은 무시하고 다음 탭부터 허용
        ...
        .pointerInteropFilter { event ->
            ...
            when (event.actionMasked) {
                MotionEvent.ACTION_POINTER_UP -> {
                    // 두 손가락 이상 제스처가 끝나고 1개 포인터만 남는 구간에서 transform 종료
                    if (event.pointerCount <= 2) {
                        resetGestureState(forceEndTransform = true)
                        suppressNextTap = true
                    }
                }

                MotionEvent.ACTION_UP -> {
                    resetGestureState(forceEndTransform = false)
                    // ACTION_UP 이후에는 다음 탭을 허용해 고착 방지
                    suppressNextTap = false
                }
